...
$table_error_message = "<span style=\"color:red;background-color:inherit;\">[FAIL]</span> Could not add necessary data to database!<br /><br /><b>A fatal MySQL error occurred</b>.\n<br />\n";
	...
	if ($conn !== false){
		...
		if (strlen($stmt)>3){
				...
				if (!$result){
					$sqlErrorCode = mysqli_connect_errno();
					$sqlErrorText = mysqli_connect_error();
					$sqlStmt      = $stmt;
					break;
				}
			}
	...
	if ($sqlErrorCode == 0){
		...
		//First section
		mysqli_query($conn, "INSERT INTO `".$mysql_database."`.`sections` (`sid`, `header`, `startedby`, `startedby_date`, `updatedby`, `updatedby_date`, `order`, `visible`, `disabled`, `deleted`) VALUES ('1', 'A section', '1', '".$date."', NULL, NULL, '1', '1', '0', '0');") or die ($table_error_message . "Error: (" . mysqli_connect_errno() . ") " . mysqli_connect_error());
		//First page
		mysqli_query($conn, "INSERT INTO `".$mysql_database."`.`content` (`id`, `sid`, `order`, `header`, `menuname`, `headline`, `context`, `startedby`, `startedby_date`, `updatedby`, `updatedby_date`, `url`, `visible`, `disabled`, `deleted`) VALUES ('1', '1', '1', 'A page', NULL, 'Test page', '<p>First page!</p>', '1', '".$date."', NULL, NULL, '0', '1', '0', '0');") or die ($table_error_message . "Error: (" . mysqli_connect_errno() . ") " . mysqli_connect_error());
		//404 page
		...
		//First user
		mysqli_query($conn, "INSERT INTO `".$mysql_database."`.`maker` (`id`, `username`, `password`, `last_login`, `todo`) VALUES ('1', '".$adminname."', MD5('".$adminpassword."'), NULL, 'Here you can write your own notes.');") or die ($table_error_message . "Error: (" . mysqli_connect_errno() . ") " . mysqli_connect_error());
		//Make him/her headadmin
		mysqli_query($conn, "INSERT INTO `".$mysql_database."`.`permissions` (`uid`, `sid`, `permissions`) VALUES ('1', '0', '1');") or die ($table_error_message . "Error: (" . mysqli_connect_errno() . ") " . mysqli_connect_error());
		mysqli_query($conn, "INSERT INTO `".$mysql_database."`.`permissions_extra` (`id`, `name`) VALUES ('1', 'Headadmin');") or die ($table_error_message . "Error: (" . mysqli_connect_errno() . ") " . mysqli_connect_error());
...
?>
	...
	<tr><th colspan="2">Welcome to the installation of Maker CMS</th></tr>
